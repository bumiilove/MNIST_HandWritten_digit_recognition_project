...
bool Adafruit_SPIDevice::write(const uint8_t *buffer, size_t len,
  ...
  if (_spi) {
    if (prefix_len > 0) {
      ...
      _spi->transferBytes((uint8_t *)prefix_buffer, (uint8_t *)nullptr, (uint32_t)prefix_len);
    }
    if (len > 0) {
      ////수정
      //_spi->transferBytes(buffer, nullptr, len);
      _spi->transferBytes((uint8_t*)buffer, (uint8_t*)nullptr, (uint32_t)len);
    }
  } else
#endif
  {
...
bool Adafruit_SPIDevice::write_then_read(const uint8_t *write_buffer,
  ...
  if (_spi) {
    if (write_len > 0) {
        ...
        _spi->transferBytes((uint8_t*)write_buffer, (uint8_t*)nullptr, (uint32_t)write_len);
    }
  } else
#endif
  {
